...
function initProjectLightbox() {
  ...
  const stageEl = lightbox.querySelector('.project-lightbox-stage');
  ...
  function normalisePhotoIndex(index) {
    return (index + photos.length) % photos.length;
  }

  function warmPhoto(index) {
    const normalisedIndex = normalisePhotoIndex(index);
    const photo = photos[normalisedIndex];
    if (!photo || !photo.src) return Promise.resolve();
    if (photoCache.has(photo.src)) return photoCache.get(photo.src).promise;

    const preloadedImage = new Image();
    preloadedImage.decoding = 'async';
    preloadedImage.src = photo.src;
    ...
    const promise = new Promise(resolve => {
      ...
      preloadedImage.decode ? preloadedImage.decode().catch(() => null) : null
    )).catch(() => null);

    photoCache.set(photo.src, { image: preloadedImage, promise });
    return promise;
  }

  function warmNearbyPhotos(index) {
    [index, index + 1, index - 1, index + 2].forEach(warmPhoto);
  }

  function setLightboxPreview(photo) {
    if (!stageEl) return;
    const previewSrc = photo.thumbSrc || photo.src;
    stageEl.style.setProperty('--lightbox-preview', previewSrc ? `url("${previewSrc.replace(/"/g, '\\"')}")` : 'none');
  }

  function showPhoto(index) {
    activeIndex = normalisePhotoIndex(index);
    const photo = photos[activeIndex];
    const currentToken = ++loadToken;

    setLightboxPreview(photo);
    lightbox.classList.add('is-loading');
    imageEl.classList.remove('is-loaded');
    imageEl.src = photo.thumbSrc || photo.src;
    imageEl.alt = photo.alt;
    captionEl.textContent = photo.alt;
    countEl.textContent = `${activeIndex + 1} / ${photos.length}`;
    requestAnimationFrame(() => imageEl.classList.add('is-loaded'));

    warmPhoto(activeIndex).then(() => {
  ...
  function closeLightbox() {
    ...
    if (stageEl) stageEl.style.removeProperty('--lightbox-preview');
  ...
  items.forEach(({ item, img }, index) => {
    ...
    item.addEventListener('mouseenter', () => warmNearbyPhotos(index), { passive: true });
    item.addEventListener('focus', () => warmNearbyPhotos(index));
    item.addEventListener('touchstart', () => warmNearbyPhotos(index), { passive: true });
  ...
  schedulePreload(() => photos.forEach((_, index) => warmPhoto(index)));
