// ============ CONFIGURATOR ============
const PRODUCT_ASSET_DIR = 'assets/products/';

function productAsset(fileName) {
  return fileName ? PRODUCT_ASSET_DIR + fileName : '';
}

const IMGS = {
  raglan: productAsset('sleeves-raglan.png'),
  setin: productAsset('sleeves-standard.png'),
  vneck: productAsset('collar-v-neck.png'),
  overlapvneck: productAsset('collar-overlap-v-neck.png'),
  round: productAsset('collar-crewneck.png'),
  polo: productAsset('collar-polo-standard.png'),
  vneckpolo: productAsset('collar-v-neck-polo.png'),
...
const PRODUCT_IMAGES = {
  // Add product photos here, for example:
  // 'home-shirt': productAsset('home-shirt.jpg'),
  // Photos are fitted inside the existing first-step frames.
  'home-shirt': productAsset('product-home-shirt.png'),
  ...
  'home-shorts': productAsset('product-home-shorts.png'),
  'away-shorts': productAsset('product-away-shorts.png'),
  'match-gaiters': productAsset('product-match-gaiters.png'),
  'goalkeeper-gaiters': productAsset('product-goalkeeper-gaiters.png'),
  'anti-slip-socks': '',
  'goalkeeper-shirt': productAsset('product-goalkeeper-shirt.png'),
  'goalkeeper-shirt-long': productAsset('product-goalkeeper-shirt-long.png'),
  'backpack': productAsset('product-backpack.png'),
  'duffel-bag': '',
  'gym-sack': '',
  'training-ball': productAsset('product-training-ball.png'),
  'match-ball': productAsset('product-match-ball.png'),
  'shin-guards': productAsset('product-shin-guards.png'),
  'club-towel': productAsset('product-club-towel.png')
};

(function preloadImages() {
  [...Object.values(IMGS), ...Object.values(PRODUCT_IMAGES)].filter(Boolean).forEach(src => {
    const img = new Image();
    img.src = src;
  });
})();
...
const CONFIG_STEP_VISIBILITY = {
  // Change a value to true to show the step again.
  sleeve: false,
  collar: false,
  colors: true,
  material: false,
...
function getActiveStepIds() {
  if (config.products.length === 0) {
    return STEP_SEQUENCE.filter(stepId => stepId === 'product' || CONFIG_STEP_VISIBILITY[stepId] !== false);
  }
  ...
  return STEP_SEQUENCE.filter(stepId => enabled.has(stepId) && (stepId === 'product' || CONFIG_STEP_VISIBILITY[stepId] !== false));
}
...
function productCard(item, groupKey) {
  ...
  const photo = item.image || PRODUCT_IMAGES[item.value] || '';
  return `
    <button class="option-card product-card${item.wide ? ' product-card-wide' : ''}${photo ? ' has-product-photo' : ''}${sel?' selected':''}"
      ...
      <div class="option-card-img-wrap product-card-visual-wrap">
        ...
        ${photo ? `<img class="product-card-photo" src="${escapeHtml(photo)}" alt="${escapeHtml(label)}" loading="eager" decoding="async" onerror="this.closest('.product-card').classList.add('product-image-missing'); this.remove();" />` : ''}
      </div>
